...
(function () {
  ...
  function applyFiltersAndSort(allResults) {
    ...
    if (q) {
      filtered = filtered.filter((r) => {
        const hay = `${r.distance} ${r.time} ${r.race} ${r.year} ${r.notes} ${r.date}`.toLowerCase();
  ...
  function renderWishlist(data) {
      ...
      const majors = (data.wishlist?.majors || []).map((m) => {
        ...
        return `
          <span class="pill ${completed ? "ok" : "no"}">
            <span class="dot"></span>
            ${safeText(m.name)} ${completed ? "✅" : "❌"}${year}
          </span>
        `;
      });
    ...
    if (otherEl) {
      ...
      otherEl.innerHTML = items.map((x) => `<li>${safeText(x)}</li>`).join("");
    }
  }
  ...
  function renderSummary(data) {
    ...
    for (const r of data.raceResults || []) {
      ...
      if (n.includes("berlin")) countries.add("Germany");
      else if (n.includes("manchester")) countries.add("UK");
      else countries.add("USA");
    }
